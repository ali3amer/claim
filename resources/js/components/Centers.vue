                    ...
                    <div class="col-6">
                        <div class="row">
                            ...
                            <div class="col-6"><input type="text" placeholder="بحث ...." class="form-control"
                                                      v-model="search" @keyup="loadData"></div>
                        </div>
                    </div>
                    <div class="col-6">
                        <div class="row">
                            <div class="col-5">
                                <div class="form-group">
                                    <select v-model="city" @change="getUnits" class="form-control">
                            ...
                            <div class="col-5">
                                <div class="form-group">
                                    <select v-model="unit" @change="loadData()" class="form-control">
                ...
                <div class="modal-content">
                    <form @submit.prevent="editMode ? updateData() : createData()">
                        ...
                        <div class="modal-body">
                            <div class="form-group">
                                <input v-model="form.name" type="text" name="name" placeholder="إسم المركز"
                                       class="form-control" :class="{ 'is-invalid': form.errors.has('name') }">
                                <has-error :form="form" field="name"></has-error>
                            </div>

                            <div class="form-group">
                                <select v-model="city" @change="getUnits" class="form-control" :class="{ 'is-invalid': form.errors.has('unit_id') }">
                                    <option value="">إختر المحليه .......</option>
                                    <option v-for="item in cities" :value="item.id">{{ item.name }}</option>
                                </select>
                            ...
                            <div class="form-group">
                                <select v-model="form.unit_id" class="form-control" :class="{ 'is-invalid': form.errors.has('unit_id') }">
                            ...
                            <div class="form-group">
                                <select v-model="form.category_id" class="form-control" :class="{ 'is-invalid': form.errors.has('unit_id') }">
                            ...
                            <div class="form-group">
                                <select v-model="form.type_id" class="form-control" :class="{ 'is-invalid': form.errors.has('unit_id') }">
...
export default {
    ...
    methods: {
        getResults(page = 1) {
            if (this.unit == 0) {
                ...
                axios.get('api/' + this.routeName + '?unit=' + this.unit + '&page=' + page)
                    .then(response => {
                        this.rows = response.data;
                    });
            }
        ...
        searchResults() {
            if (this.search != '') {
                axios.get('api/' + this.routeName + '?name=' + this.search + '&unit=' + this.unit).then(({data}) => (this.rows = data));
            } else if (this.search == '') {
        ...
        editModal(row) {
            ...
            $("#" + this.modalTitle).modal('show');
            this.form.fill(row);
            this.city = this.allUnits[row.unit_id].city_id;
            this.getUnits();
        },
        deleteData(id) {
            swal.fire({
    ...
    created() {
        axios.get('api/city?select=all').then(({data}) => (this.cities = data));
        axios.get('api/unit?select=all').then(({data}) => (this.allUnits = data));
        axios.get('api/category?select=all').then(({data}) => (this.categories = data));
        axios.get('api/type?select=all').then(({data}) => (this.types = data));
        this.loadData();
