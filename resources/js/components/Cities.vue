                        ...
                        <div class="row">
                            <div class="col-8">
                                <div class="form-group">
                                    <select v-model="state" disabled="permissions.power != 'developer'" @change="loadData()" class="form-control">
                                        <option value="">إختر ولايه .......</option>
                ...
                <div class="modal-content">
                    <form @submit.prevent="editMode ? updateData() : createData()">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="form-group">
                                <select v-model="form.state_id" disabled="permissions.power != 'developer'" class="form-control" :class="{ 'is-invalid': form.errors.has('state_id') }">
                                    <option value="">إختر ولايه .......</option>
                                    <option v-for="item in states" :value="item.id">{{ item.name }}</option>
                                </select>
...
export default {
    data() {
        return {
            ...
            rows: {},
            states: {},
            state: this.permissions.state_id,
            form: new Form({
                id: '',
                name: '',
                state_id: this.permissions.state_id,
            })
        }
    },
    ...
    methods: {
        getResults(page = 1) {
            if (this.state == 0) {
                ...
                axios.get('api/' + this.routeName + '?state=' + this.state + '&page=' + page)
                    .then(response => {
                        this.rows = response.data;
                    });
            }
        ...
        loadData() {
            if (this.state != 0) {
                axios.get('api/'+ this.routeName + '?state=' + this.state).then(({data}) => (this.rows = data));
            } else {
                axios.get('api/'+ this.routeName).then(({data}) => (this.rows = data));
            }
        },
    ...
    created() {
        ...
        this.loadData();
        axios.get('api/state?select=all').then(({data}) => (this.states = data));
